...
class Main:
	...
	def getVideos( self ) :
		...
		# Get HTML page...
		# 		
		usock    = urllib.urlopen( "http://www.gametrailers.com/game-previews?page=%u&rdir=1" % ( (self.current_page - 1 ) * 10 ) )
		htmlData = usock.read()
		usock.close()
		...
		for div_content_row_super in divs_content_row_super:
			...
			# Thumbnail overlay (HD or not)...
			overlay = ( xbmcgui.ICON_OVERLAY_NONE, xbmcgui.ICON_OVERLAY_HD ) [ "-hd-" in video_page_url ] 

			# Title...
			a_gamepage_content_row_title = div_content_row_super.find( "a", { "class" : "gamepage_content_row_title" } )
			title = a_gamepage_content_row_title.string.strip()
			
			# Date...
			span_gamepage_content_row_date = div_content_row_super.find( "span", { "class" : "gamepage_content_row_date" } )
			date = span_gamepage_content_row_date.string.strip()
			
			# Plot...			
			span_gamepage_content_row_text = div_content_row_super.find( "span", { "class" : "gamepage_content_row_text" } )
			plot = span_gamepage_content_row_text.string.strip() 
		...
		xbmcplugin.addDirectoryItem( handle = int(sys.argv[1]), url = "%s?action=list-previews&plugin_category=%s&page=%i" % ( sys.argv[0], self.plugin_category, self.current_page + 1 ), listitem = listitem, isFolder = True)
